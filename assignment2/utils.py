...
def draw_correspondences(img, ptsTrue, ptsReproj, ax, drawOnly=50): 
    ...
    row_total = ptsTrue.shape[0]
    row_seq = np.arange(row_total)
    np.random.shuffle(row_seq)
    ptsTrue, ptsReproj = ptsTrue[row_seq[:drawOnly]], ptsReproj[row_seq[:drawOnly]]

    ax.scatter(ptsTrue[:, 0], ptsTrue[:, 1], c='r')
    ax.scatter(ptsReproj[:, 0], ptsReproj[:, 1], c='b')
    return ax