...
class SFM(object): 
    ...
    def trangulate_new_view(self, name): 
        ...
        for prev_name in self.image_data.keys(): 
            if prev_name != name: 
                ...
                # curr_name_ref = self.image_data[name][-1]
                # matches = [match for match in matches if curr_name_ref[match.trainIdx] < 0]
                ...
                if len(matches) > 0:
                    ...
                    # Remove outliers
                    _, mask = cv2.findEssentialMat(img1pts, img2pts, self.K, threshold=0.9)
                    mask = mask.squeeze().astype(bool)
                    img1pts, img2pts, img1idx, img2idx = img1pts[mask], img2pts[mask], img1idx[mask], img2idx[mask]
    ...
    def compute_reprojection_error(self, name): 
        ...
        kp = np.array(kp)[ref >= 0]
        img_pts = np.array([_kp.pt for _kp in kp])

        reproj_pts, _ = cv2.projectPoints(self.point_cloud, R, t, self.K, 0)
        reproj_pts = reproj_pts.squeeze()

        idx = [int(r) for r in ref if r >= 0]
        reproj_pts = reproj_pts[idx]
        err = np.mean(np.linalg.norm(reproj_pts - img_pts, axis=1))
